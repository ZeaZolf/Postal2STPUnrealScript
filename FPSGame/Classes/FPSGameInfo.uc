...
///////////////////////////////////////////////////////////////////////////////
function int PlayMusicExt(string Song, float FadeInTime)
	{
	local int i;
	local int handle;
	...
	i = GetFreeSongInfo();
	SongInfos[i].RealHandle = PlayMusic(Song, FadeInTime);
...
function int PlayMusicAttenuateExt(actor Actor, string Song, float FadeInTime, optional float Volume, optional float Radius, optional float Pitch)
	{
	local int i;
	local int handle;
	...
	if (Actor != None)
		...
		// Pass it on to the real function, preserving the info we need to restore song if necessary
		i = GetFreeSongInfo();
		SongInfos[i].RealHandle = Actor.PlayMusicAttenuate(Song, FadeInTime, Volume, Radius, Pitch);
		if (SongInfos[i].RealHandle > 0)
			{
	...
	for (i = 0; i < ArrayCount(SongInfos); i++)
		...
		if (SongInfos[i].RealHandle > 0 || SongInfos[i].RealHandle == SPECIAL_RESTORATION_HANDLE)
			...
			if (SongInfos[i].Actor == None)
				...
				SongInfos[i].RealHandle = SongInfos[i].Actor.PlayMusicAttenuate(SongInfos[i].Song, 0.1, SongInfos[i].Volume, SongInfos[i].Radius, SongInfos[i].Pitch);
