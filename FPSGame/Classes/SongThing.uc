...
function StartMusic()
	{
	if (bAttenuate)
		MusicHandle = FPSGameInfo(Level.Game).PlayMusicAttenuateExt(self, SongName, FadeInTime, AttenuateVolume, AttenuateRadius, 1.0);
	else
		MusicHandle = FPSGameInfo(Level.Game).PlayMusicExt(SongName, FadeInTime);

	if (MusicHandle != 0)
		bChannelInUse = true;

//	Log("SongThing.StartMusic(): MusicHandle="$MusicHandle);
	}
