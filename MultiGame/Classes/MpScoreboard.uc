...
function DrawPlayerNetInfo(Canvas Canvas, MpPlayerReplicationInfo PRI, float X, float Y)
{
	...
	if (Level.NetMode != NM_Standalone)
	{
		...
		if (NetRows > 1)
		{
			...
			Canvas.SetPos(X + Canvas.ClipX * PlayerNetPX, CenterY);
			MyFont.DrawText(Canvas, TimeString @ str);
			CenterY += PlayerFontSmYL;
		}
...
defaultproperties
{
	...
	DeadFireText="You were killed.  Press FIRE to restart."
	OutFireText="You are OUT.  Press FIRE to view other players."
	WaitingToSpawn="Press FIRE to join the match!"
	InitialViewingString="Press FIRE to track other players or ALT-FIRE for free cam"
	StartFireText="Press FIRE to start"
	ReadyText="READY"
	NotReadyText="NOT"
	OutText="OUT"
	ForNextText="Press ALT-FIRE for next tip."
	ToggleScoreText="Press F1 (default) to toggle scoreboard."
