...
event Initialized()
	...
	InstalledDescription = InstalledTitle$": "$InstalledType$"-"$InstalledCountry;
...
function PostRender(canvas Canvas)
	...
	else if(bShowedLoadTexture && CAPS(CurrentLevel) != CAPS("Startup"))
	{
	...
	*/
		...
		if (MyMenu.class == class'Menumain' || MyMenu.class == class'MenuGame')
			{
			MyFont.DrawTextEx(
				Canvas,
				Canvas.ClipX,
...
function SetLoadingTexture(string MapName)
{
	...
	if(LevSumObject != None && LevelSummary(LevSumObject) != None && Caps(MapName) != Caps("startup") && Caps(MapName) != Caps("entry") && Caps(MapName) != Caps("index"))
	{
	...
	else if( (LevSumObject == None || LevelSummary(LevSumObject) == None)  && Caps(MapName) != Caps("startup") && Caps(MapName) != Caps("entry") && Caps(MapName) != Caps("index"))
		LoadingTexture = Default.LoadingTexture;
}
...
defaultproperties
	...
	ArrestedDemoMenu=class'MenuDemoArrested'
