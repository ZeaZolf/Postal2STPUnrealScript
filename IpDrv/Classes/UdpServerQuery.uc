...
function string GetBasic() {
	...
	ResultSet = ResultSet$"\\location\\"$class'UdpServerUplink'.default.ServerRegion;
...
function string GetInfo() {
	...
	// RWS CHANGE: show game type's game name, not class name
	ResultSet = ResultSet$"\\gametype\\"$GetItemName(Level.Game.GameName);
...
function string GetPlayer( PlayerController P, int PlayerNum )
{
	...
	if(P.PlayerReplicationInfo.Team != None)
		ResultSet = ResultSet$"\\team_"$PlayerNum$"\\"$P.PlayerReplicationInfo.Team.TeamName;
	else
		ResultSet = ResultSet$"\\team_"$PlayerNum$"\\None";

	// Character
	ResultSet = ResultSet$"\\character_"$PlayerNum$"\\"$P.PawnClass.Default.MenuName;
	//if(P.Pawn != None)
	//	ResultSet = ResultSet$"\\character_"$PlayerNum$"\\"$P.Pawn.MenuName;
	//else
	//	ResultSet = ResultSet$"\\character_"$PlayerNum$"\\Spectator";

	// RWS CHANGE: Updated var name for 927
