...
function Trigger( actor Other, Pawn EventInstigator )
{
	...
	if(!MyPawn.bPlayerIsFriend)
	{
		...
		{
			// Run really far away from where you are right now
			DangerPos = Location;
	...
	if(p2weap != None)
	{
			...
			&& !P2Pawn(LookAtMe).bAuthorityFigure)
		{
			...
			if(dist < p2weap.RecognitionDist)
			{
				...
				else if(CowheadBaseWeapon(LookAtMe.Weapon) != None
...
function ConvertToRiotMode()
{
	...
	MyPawn.HealthMax = RIOT_HEALTH_INCREASE*MyPawn.default.HealthMax;
	MyPawn.Health = MyPawn.default.HealthMax;
	MyPawn.DamageMult = RIOT_DAMAGE_INCREASE*MyPawn.default.DamageMult;
