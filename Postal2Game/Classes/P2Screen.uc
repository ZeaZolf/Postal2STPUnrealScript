...
function DrawScaled(
	...
	if (bEnableRender)
		{
		Canvas.SetPos(BackX * ScaleX, BackY * ScaleY);
		Canvas.DrawTile(tex, tex.USize * ScaleX, tex.VSize * ScaleY, 0, 0, tex.USize, tex.VSize);
		}
	}
...
state FadeInScreen extends ShowScreen
	...
	function BeginState()
		...
		if (FadeInScreenSound != None)
			GetSoundActor().PlaySound(FadeInScreenSound);
		if (Song != "")
			SongHandle = GetGameSingle().PlayMusicExt(Song, FadeInSongTime);
		if (bFadeScreenInOut)
...
state FadeOutScreen extends ShowScreen
	...
	function BeginState()
		...
		if (FadeOutScreenSound != None)
			GetSoundActor().PlaySound(FadeOutScreenSound, SLOT_Misc);
