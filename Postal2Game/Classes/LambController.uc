...
const MIN_DIST_FROM_PLAYERS_FOR_STASIS	=	2000; // All players must be at least this far from an AI character
...
function PreTeleportWithPlayer(out FPSGameState.TeleportedPawnInfo info, P2Pawn PlayerPawn)
{
	...
	info.CurrentSkin = Texture(Pawn.Skins[0]);
...
state LegMotionToTarget
{
	...
	event HitPathGoal(Actor Goal, vector Dest)
	{
		...
		//log(Pawn$" hit goal %%%%%%%%%%%"$Goal$" end goal "$EndGoal$" my dest "$dest);
		/*
		if(ApproxGoal == Goal
			&& ApproxGoal != None)
		{
