...
var Texture BurnSkin;			// what I switch to after I die from fire
						   ...
						   Vector hitlocation)
{
	...
	// Don't let friendly fire hurt your armor
	if(IsPlayerPawn()
		&& instigatedby.IsPlayerPawn()
		&& PlayerReplicationInfo.Team != None	// Make sure we're using teams
		&& PlayerReplicationInfo.Team == instigatedby.PlayerReplicationInfo.Team)
	...
	else if(damageType == class'BludgeonDamage')
	{
	...
	else if(damageType == class'KickingDamage')
	{
		// armor blocks a lot of kicking damage
		ArmorDamage = Damage*ARMOR_KICKING_BLOCK;
	}
	else if(damageType == class'SmashDamage')
	{
						...
						Vector momentum, class<DamageType> damageType)
{
	...
	//log(self$" TakeDamage d "$Damage$" p "$instigatedBy$" hit "$HitLocation$" mom "$Momentum$" type "$DamageType$" state "$GetStateName()$" god mode "$Controller.bGodMode);
	...
	if(Armor > 0
		...
		&& (Controller == None
			|| !Controller.bGodMode))
		ArmorAbsorbDamage(instigatedby, Damage, DamageType, HitLocation);
